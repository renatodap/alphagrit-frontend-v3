...
/// Persistent macro calculator that saves results to backend
class PersistentMacroCalculator extends StatefulWidget {
  final int programId;
  final WinterArcRepository repository;

  const PersistentMacroCalculator({
    super.key,
  ...
  @override
  State<PersistentMacroCalculator> createState() => _PersistentMacroCalculatorState();
}

class _PersistentMacroCalculatorState extends State<PersistentMacroCalculator> {
  ...
  Future<void> _loadSavedData() async {
    ...
    try {
      ...
      // Load form inputs
      if (data['current_weight_kg'] != null) {
        _weightController.text = data['current_weight_kg'].toString();
      }
      ...
      setState(() {
        _bmr = data['bmr']?.toDouble();
        _tdee = data['tdee']?.toDouble();
        _targetCalories = data['target_calories']?.toDouble();
        _protein = data['protein_g']?.toDouble();
        _fats = data['fat_g']?.toDouble();
        _carbs = data['carbs_g']?.toDouble();
        _isLoading = false;
      });
    } catch (e) {
      setState(() {
  ...
  Future<void> _saveResults() async {
    ...
    try {
      await widget.repository.updateProgress(
        widget.programId,
        {
          'current_weight_kg': double.parse(_weightController.text),
